...
def create_info_table(table_name, attributes, comments, table_prefix, custom_column_type={}):
...
def create_table_by_category(tables, comments, db_prefix=config.db_prefix, table_prefix=config.table_prefix,
                             custom_column_type={}):
        ...
        for table in tables[category]:
            ...
            sql_commands.append(
                create_secondary_table(f"{table_prefix}{category.lower()}_{table}", tables[category][table],
                                       ...
                                       f"{table_prefix}{category.lower()}_info", custom_column_type=custom_column_type_dict))
    return sql_commands
...
def create_relation_table(tables_dict, comments_dict, table_prefix=config.table_prefix, custom_column_type={}):
    ...
    # return a list of sql commands
    # FOR RELATIONS ONLY
    # print(tables_dict, comments_dict)
    sql_commands = []
            ...
            for table in tables_dict[general_category][category].keys():
                ...
                for i, attribute in enumerate(tables_dict[general_category][category][table]):
                    ...
                    if "date" in attribute.lower():
                        ...
                        sql += f"`{attribute}` VARCHAR(256) {comment_str}, "
                    elif attribute.lower() in custom_column_type_dict.keys():
                        sql += f"`{attribute}` {custom_column_type_dict[attribute.lower()]} {comment_str}, "
                    else:
                        sql += f"`{attribute}` VARCHAR(32) {comment_str}, "
                ...
                for i, party in enumerate(parties):
                    sql += f", FOREIGN KEY ({foreign_key_names[i]}_id) REFERENCES {table_prefix}{party}_info({party}_id)"
                sql += ");"
